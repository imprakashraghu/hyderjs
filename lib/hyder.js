...
class HyderUtil extends HyderError {
    constructor() {
        super();
    }
    _AttachUniqueKey = (content, container) => {        
        if(!content.id) {
            let uid = (Date.now() + Math.random()).toString(36).replace('.','#');     
            content.id = uid;   
            container.id = uid;
        }        
        return container;
    }         
    _CreatingElement = (root, i) => {        
        if(i.type === "heading") {
            root.append(this._Heading(i.data));
        }
        if(i.type === "text") {
            root.append(this._Paragraph(i.data));
        }
        if(i.type === "link") {
            root.append(this._Anchor(i.data));
        }
        if(i.type === "input") {
            root.append(this._Input(i.data));
        }
        if(i.type === "button") {
            root.append(this._Button(i.data));
        }
        if(i.type === "list") {
            root.append(this._List(i.data));
        }
        if(i.type === "dropdown") {
            root.append(this._Dropdown(i.data));
        }
        if(i.type === "image") {
            root.append(this._Image(i.data));
        }
        if(i.type === "container") {
            root.append(this._HyderContainer(i.data));
        }
        if(i.type === "break") {
            root.append(this._Break(i.data));
        }
        if(i.type === "form") {
            root.append(this._Form(i.data));
        }
        if(i.type === "card") {
            root.append(this._Card(i.data));
        }
        if(i.type === "modal") {
            root.append(this._Modal(i.data));
        }
...
class Component extends aggregation(HyderUtil,StyleProps) {           
    ...
    _HyderContainer(content) {
        ...
        if(content.items.length) {            
            ...
            content.items.map(i => this._CreatingElement(tag, i));
        }
        return tag;
    }
    _Break(content) {
...
class HyderJS extends Component{
    constructor(obj) {
        ...
        this.inputType = obj.type || null;                      
        this._defaultPageStyling();                
        this._renderComponents();
    }               
    _renderComponents() {      
        this.bricks.map(i => {            
            this._CreatingElement(this.parent, i);                        
        });                 
    }            
    _defaultPageStyling() {
